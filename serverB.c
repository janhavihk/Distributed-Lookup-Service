...
//create socket
if ((serverB_socket = socket(AF_INET, SOCK_DGRAM, 0)) < 0)
{
...
serverB_address.sin_port = htons(MYPORT);//server B on port 22837
serverB_address.sin_addr.s_addr=inet_addr("127.0.0.1");
memset(serverB_address.sin_zero,'\0',sizeof serverB_address.sin_zero);//clear memory for server address

//Binding the server address to the socket
if(bind(serverB_socket, (struct sockaddr *)&serverB_address, sizeof(serverB_address))<0)
{
...
if(recvfrom(serverB_socket,(struct received_data *)&result, sizeof(result),0,(struct sockaddr *)&client_information, &p)<0)
{
	perror("Error recieving data:");
	exit(3);
}
printf("The ServerB received input <%s> and operation <%s>",result.word,result.function);
...
if(strcmp(result.function,"search")==0){
printf("\nThe serverB has found <%d> match and <%d> similar words\n",data.count[0],countForSimilar);
}
if(strcmp(result.function,"prefix")==0){
printf("\nThe serverB has found <%d> match\n",data.count[2]);
}
...
//sending data
//printf("\nprefix results:\n%s\n search result:\n %s\nsuffix results:\n%s",data.prefixWord,data.searchWord,data.suffixWord);
if (sendto(serverB_socket,(struct send_data *)&data,sizeof(data),0,(struct sockaddr *)&client_information,sizeof(client_information)) < 0)
{
	perror("send error");
	close(serverB_socket);
	exit(4);
